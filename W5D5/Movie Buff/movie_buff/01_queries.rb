...
def harrison_ford
  ...
  Movie.joins(:actors).joins(:castings).where("actors.name like ?","Harrison Ford").where("castings.ord != ?",1).select(:id, :title).distinct
...
def directed_by_one_of(them)
  ...
  #Actor.joins(:directed_movies).where(name: ['George Lucas','Steven Spielberg']).select("movies.id", :"movies.title")
